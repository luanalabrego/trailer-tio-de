...
export default function CardapioPage() {
  ...
  // carrega produtos, clientes e tenta pré-login do cliente
  useEffect(() => {
    async function init() {
  ...
  function onTelefoneChange(val: string) {
    ...
    if (cli) {
      ...
      localStorage.setItem('clienteTelefone', val)
  ...
  async function handleAgendar() {
    ...
    if (confirm('Pedido confirmado!\n\nDeseja enviar via WhatsApp?')) {
      ...
      const texto = [`Olá ${payload.nome}, seu pedido:`, linhas, `Total: R$ ${payload.total.toFixed(2)}`, `Agendado para: ${new Date(payload.dataHora).toLocaleString('pt-BR')}`].join('\n\n')
      window.open(`https://wa.me/55${telefone.replace(/\D/g,'')}?text=${encodeURIComponent(texto)}`, '_blank')
    }
    // clear
    setCarrinho([])
    setDataHoraAgendada('')
        ...
        : (
          <div className="bg-white p-4 rounded-xl shadow border">
            <div className="flex justify-between items-center mb-4">
              <span className="font-semibold">Cliente: {clienteExistente?.nome || nome}</span>
              <button onClick={mudarTelefone} className="text-sm text-indigo-600 hover:underline">
                Mudar número
              </button>
            </div>
            {carrinho.length === 0 ? (
              ...
              <>
                ...
                <div className="text-right font-bold text-lg mb-4">Total: R$ {total.toFixed(2)}</div>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded-xl p-6 w-full max-w-md shadow-xl">
            ...
            <div className="space-y-4">
              <div>
                <label className="block mb-1">WhatsApp (com DDD)</label>
                <input
                  type="tel"
                  value={telefone}
              ...
              <div className="flex justify-end gap-2 mt-4">
                ...
                <button onClick={handleConfirmarCliente} className="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">
                  Continuar
                </button>
              </div>
            </div>
